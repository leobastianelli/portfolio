...
export default function Stack() {
  ...
  return (
    <section id="stack" className="py-9 md:py-10 px-5 md:px-6" style={{ borderTop: "1px solid var(--border)" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-7">
          {t.stack.groups.map((group, i) => (
            <div key={group.category} className="stack-card reveal pt-5" style={{ transitionDelay: `${i * 0.06}s` }}>
              <p className="font-mono" style={{ fontSize: "var(--text-2xs)", letterSpacing: "0.22em", textTransform: "uppercase", marginBottom: "1.2rem", color: "var(--color-accent-ink)" }}>
                {group.category}
              </p>

              <ul className="flex flex-col gap-2">
                {group.items.map((item) => (
                  <li key={item} className="font-body flex items-center gap-2" style={{ fontSize: "var(--text-sm)", color: "var(--color-ink)" }}>
